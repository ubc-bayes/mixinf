...
# ARG PARSE SETTINGS ####
parser = argparse.ArgumentParser(description="plot results from normal-kernel sequential-opt variational mixture inference")
...
for N in ss:
    ...
    for file in files:
        ...
        qN = q(t[:, np.newaxis])
        if i == 1: plt.plot(t, qN, 'c-', label = 'mixture', linewidth = 1, markersize = 0.1, alpha = 0.5)
        else: plt.plot(t, qN, 'c-', linewidth = 1, markersize = 0.1, alpha = 0.5)
        i += 1
