...
def mixture_rvs(size, w, x, rho):
    ...
    ind = np.random.choice(N, size = size, p = w, replace = True) # indices that will be chosen
    ...
    sigmas = rho[ind] # index std deviations for ease
    return rand * np.sqrt(sigmas[:, np.newaxis]) + x[ind]
###########
...
###########
def nfvmi(p, x, type = 'kl', tol = 1e-3, maxiter = 1e3, B = 500, b = 0.1, silent = False, trace = True, path = ''):
    """
